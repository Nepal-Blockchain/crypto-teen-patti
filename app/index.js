...
let TeenPatti, accounts, account, _hash
let net
let stat = [
...
const App = {
  ...
  networkCheck: async () => {
    try {
      App.netID = await infura.eth.net.getId()
      if ((ABI.networks).hasOwnProperty(App.netID)) {
        let tx = await infura.eth.getTransaction(ABI.networks[App.netID].transactionHash)
        if (tx !== null) {
          App.deployBlock = tx.blockNumber
          App.TeenPatti = new web3.eth.Contract(ABI.abi, ABI.networks[App.netID].address)
          App.iPatti = new infura.eth.Contract(ABI.abi, ABI.networks[App.netID].address)
      ...
      throw ("Contract not deployed on network " + net + "<br> Download Metamask plugin & switch to Rinkeby Testnet")
    } catch (e) {
      App.onError(e)
    }
  },
  ...
  logBook: () => {
    ...
    App.GameLog = App.iPatti.events.NewGame({
      ...
      .on('data', (event) => {
        ...
        <td id="houseCard_${_m.gameID}" class="tinyCard"><i class="card_0_0"></i><i class="card_0_0"></i><i class="card_0_0"></i></td>
        <td id="playerCard_${_m.gameID}" class="tinyCard"><i class="card_0_0"></i><i class="card_0_0"></i><i class="card_0_0"></i></td>
  ...
  refreshBalance: async () => {
    try {
      ...
      document.getElementById('gameBal').innerHTML = App.toSzabo(await infura.eth.getBalance(ABI.networks[App.netID].address))
  ...
  estimatePlay: () => {
    ...
    let s = side[0].checked ? false : true
    try {
      App.iPatti.methods.play(s).estimateGas({
        from: account,
  ...
  play: () => {
    ...
    try {
      App.iPatti.methods.play(s).estimateGas({
        ...
        App.TeenPatti.methods.play(s).send({
            from: account,
            gas: est,
            gasPrice: gas * 1000000000,
            value: web3.utils.toWei(amount, 'szabo')
...
window.addEventListener('load', async () => {
  try {
    ...
    window.infura = new Web3(new Web3.providers.WebsocketProvider('wss://rinkeby.infura.io/ws/v3/5070fb8309944f18aa07840da75a9da9'))
    App.start()
  } catch (e) {
    App.onError(e)
  }
